...
let eval_store_insns op rs1 offset rs2 rfile mem =
  ...
  else if addr mod 4 <> 0 then raise NotWordAligned (* TODO: Store byte*)
  else
...
let process_utype rd imm rfile = let open Int32 in let v = shift_right (of_string imm) 12 in update_register rd (Int32.to_int v) rfile; rfile
